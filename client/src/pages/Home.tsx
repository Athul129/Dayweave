/* Dayweave style reminder: Quiet Cartography â€” editorial wayfinding, tactile paper, warm ink, offset composition, and humane pacing. */
import { useEffect, useMemo, useRef, useState } from "react";
import { ArrowDownRight, ArrowUpRight, CalendarDays, Check, ChevronRight, Clock3, Compass, Feather, Inbox, LayoutDashboard, Leaf, Menu, MoreHorizontal, Pencil, Play, Plus, Save, Sparkles, SunMedium, Timer, Trash2, Wind, X } from "lucide-react";
...
type FocusSession = { taskId: string; durationSeconds: number; startAt: number; endAt: number; pausedAt: number | null; pausedRemainingSeconds: number | null; isRunning: boolean; completed: boolean; updatedAt: number };
...
const formatWeekRange = (weekDays: WeekDay[]) => { const first = parseDateOnly(weekDays[0].date); const last = parseDateOnly(weekDays[weekDays.length - 1].date); const firstMonth = new Intl.DateTimeFormat("en-US", { month: "long" }).format(first); const lastMonth = new Intl.DateTimeFormat("en-US", { month: "long" }).format(last); const firstYear = first.getFullYear(); const lastYear = last.getFullYear(); if (firstYear === lastYear && firstMonth === lastMonth) return `${firstMonth} ${first.getDate()} â€” ${last.getDate()}, ${firstYear}`; if (firstYear === lastYear) return `${firstMonth} ${first.getDate()} â€” ${lastMonth} ${last.getDate()}, ${firstYear}`; return `${firstMonth} ${first.getDate()}, ${firstYear} â€” ${lastMonth} ${last.getDate()}, ${lastYear}`; };
...
const resumeFocusSession = (session: FocusSession, now = Date.now()): FocusSession => { const remainingSeconds = Math.max(0, Math.ceil(session.pausedRemainingSeconds ?? getFocusRemainingSeconds(session, now))); return { ...session, startAt: now, endAt: now + remainingSeconds * 1000, pausedAt: null, pausedRemainingSeconds: null, isRunning: remainingSeconds > 0, completed: remainingSeconds === 0, updatedAt: now }; };
...
function loadFocusSession(tasks?: Task[]): FocusSession | null {
  try {
    ...
    return { taskId: String(saved.taskId), durationSeconds, startAt, endAt, pausedAt, pausedRemainingSeconds, isRunning: !completed && pausedAt === null, completed, updatedAt: now };
  } catch { return null; }
}
...
function TaskForm({ mode, initial, weekDays, onSave, onCancel, onRequestDelete, deleteConfirm, onConfirmDelete, onCancelDelete }: { mode: "create" | "edit"; initial: TaskDraft; weekDays: WeekDay[]; onSave: (draft: TaskDraft) => void; onCancel: () => void; onRequestDelete?: () => void; deleteConfirm?: boolean; onConfirmDelete?: () => void; onCancelDelete?: () => void }) {
  ...
  return <form className="task-form" onSubmit={submit} noValidate>
    ...
    {deleteConfirm && <div className="delete-confirm"><div className="delete-icon"><Trash2 size={17} /></div><div><strong>Delete this task?</strong><p>This canâ€™t be undone.</p></div><div className="delete-actions"><button type="button" className="cancel-delete" onClick={onCancelDelete}>Keep it</button><button type="button" className="confirm-delete" onClick={onConfirmDelete}>Delete</button></div></div>}
...
function NoteForm({ mode, initial, onSave, onCancel, onRequestDelete, deleteConfirm, onConfirmDelete, onCancelDelete }: { mode: "create" | "edit"; initial: NoteDraft; onSave: (draft: NoteDraft) => void; onCancel: () => void; onRequestDelete?: () => void; deleteConfirm?: boolean; onConfirmDelete?: () => void; onCancelDelete?: () => void }) {
  ...
  return <form className="note-form" onSubmit={submit} noValidate>
    ...
    {deleteConfirm && <div className="delete-confirm"><div className="delete-icon"><Trash2 size={17} /></div><div><strong>Delete this note?</strong><p>This canâ€™t be undone.</p></div><div className="delete-actions"><button type="button" className="cancel-delete" onClick={onCancelDelete}>Keep it</button><button type="button" className="confirm-delete" onClick={onConfirmDelete}>Delete</button></div></div>}
...
function FocusMode({ task, session, setSession, onComplete, onExit, onAddTask, onReturnToday }: { task?: Task; session: FocusSession | null; setSession: React.Dispatch<React.SetStateAction<FocusSession | null>>; onComplete: () => void; onExit: () => void; onAddTask: () => void; onReturnToday: () => void }) {
  ...
  return <div className="focus-overlay" role="dialog" aria-modal="true" aria-labelledby="focus-title" ref={overlayRef}><div className="focus-topline"><span className="focus-brand"><div className="focus-brand-mark"><img src="/images/dayweave-mark.webp" alt="" /></div><strong>dayweave</strong></span><span className="eyebrow">ONE THING Â· {task.time}</span><button ref={closeButtonRef} className="focus-exit" onClick={() => setExitConfirm(true)} aria-label="Exit focus mode"><X size={19} /> Exit</button></div><main className="focus-content"><div className="focus-route-note"><span className="focus-route-line" /><span>{task.section.toUpperCase()} Â· {task.energy.toUpperCase()} ENERGY</span><span className="focus-route-line" /></div><h1 id="focus-title">{task.title}</h1><p className="focus-note">{task.note}</p><div className={`focus-timer-wrap ${finished ? "focus-finished" : ""}`}><div className="focus-timer-progress" style={{ "--focus-progress": `${percent * 3.6}deg` } as React.CSSProperties}><div className="focus-timer-inner" role="status" aria-live="polite"><strong>{finished ? "Done" : formatCountdown(remainingSeconds)}</strong><span>{finished ? "TIME WELL SPENT" : beforeStart ? "NOT STARTED" : `${formatMinutes(task.minutes)} ESTIMATED`}</span></div></div></div>{finished ? <div className="focus-complete-copy"><span className="eyebrow accent"><Check size={13} /> BLOCK COMPLETE</span><h2>You stayed with it.</h2><p>The task is still here until you choose what comes next.</p><button className="primary-action" data-focus-complete="true" onClick={onComplete}><Check size={15} /> Mark complete</button><button className="focus-extend" onClick={() => setSession((current) => current ? resumeFocusSession({ ...current, pausedAt: Date.now(), pausedRemainingSeconds: 5 * 60, completed: false }, Date.now()) : current)}>Continue for 5 more minutes</button></div> : <div className="focus-controls"><div className="focus-control-meta"><span>{percent}% of this block</span><span>{task.energy} energy Â· {formatMinutes(task.minutes)}</span></div><div className="focus-control-buttons"><button className="focus-pause" disabled={beforeStart} onClick={() => setSession((current) => current ? (current.pausedAt === null ? pauseFocusSession(current) : resumeFocusSession(current)) : current)} aria-label={currentSessionLabel(session, beforeStart)}>{currentSessionLabel(session, beforeStart)}</button><button className="primary-action focus-complete-button" data-focus-complete="true" onClick={onComplete}><Check size={16} /> Complete</button></div></div>}</main>{exitConfirm && <div className="focus-confirm" role="alertdialog" aria-modal="true" aria-labelledby="leave-focus-title"><span className="eyebrow accent">KEEP THE THREAD</span><h2 id="leave-focus-title">Leave focus?</h2><p>Your progress will be kept.</p><div><button className="secondary-action" onClick={() => setExitConfirm(false)}>Keep focusing</button><button className="primary-action" onClick={onExit}>Leave focus</button></div></div>}</div>;
}
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    {toast && <div className="toast" role="status" aria-live="polite"><Check size={16} /> {toast}</div>}</div>;
}
