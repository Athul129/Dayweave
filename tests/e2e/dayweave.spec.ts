...
const weekBreadcrumb = (date: Date) => { const end = addDays(date, 6); const startMonth = date.toLocaleDateString("en-US", { month: "long" }); const endMonth = end.toLocaleDateString("en-US", { month: "long" }); const year = end.getFullYear(); return startMonth === endMonth && date.getFullYear() === year ? `${startMonth} ${date.getDate()} — ${end.getDate()}, ${year}` : `${startMonth} ${date.getDate()} — ${endMonth} ${end.getDate()}, ${year}`; };
...
test.describe("Focus Mode", () => {
  test("starts, pauses, resumes, exits safely, and completes the shared task", async ({ page }) => {
    ...
    await page.clock.setFixedTime(new Date(2030, 0, 15, 11, 40, 0));
    ...
    await page.clock.setFixedTime(new Date(2030, 0, 15, 11, 40, 3));
